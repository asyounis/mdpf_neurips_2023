...
class UAV123FullSequenceEvaluation(FullSequenceEvaluationVideoBase):
    ...
    def render_frame(self, frame_number, data, output_dicts, axes):
        ...
        # Render the state
        self.draw_xy_state(frame_number, axes[1, 0], data, output_dicts, "State Output Center")
        self.draw_width_height_state(frame_number, axes[2, 0], data, output_dicts, "State Output Width/Height")
        # # Render the angle
        ...
        if(particle_weights is not None):
            self.add_ess_to_plot(axes[0, 1], particle_weights, "State Output")
        ...
        if(self.model.outputs_kde()):
            ...
            if(self.model.decouple_bandwidths_for_resampling):
                ...
                self.plot_bandwidths(frame_number, axes[2, 2], resampling_bandwidths, "Resampling Bandwidths")
        ...
        if(self.model.decouple_weights_for_resampling):
            ...
            if(resampling_particle_weights is not None):
                self.add_ess_to_plot(axes[1, 1], resampling_particle_weights, "Resampling")
        ...
        if(self.model.decouple_weights_for_resampling or self.model.decouple_bandwidths_for_resampling):
            ...
            self.draw_width_height_state(frame_number, axes[1, 2], data, output_dicts, "Resampling Output Width/Height","resampling_")
    ...
    def draw_width_height_state(self, frame_number, ax, data, output_dicts, title, key_name_prefix=""):
        ...
        if(self.model.outputs_kde() or (self.model.outputs_particles_and_weights() and self.use_manual_bandwidth)):
            ...
            ranges = [[0, 10], [0, 10]]
            ax.hist2d(width_samples, height_samples, range=ranges, bins=200, cmap="Blues", alpha=1.0)
