...
class DeepMindMazeProposalEvaluations(ProposalEvaluationBase):
    ...
    def render_experiment(self, ax, output_dict, observations, states):
        ...
        # sensors = self.evaluation_dataset.get_senssors()
        ...
        true_state_circle = plt.Circle((starting_x, starting_y), 0.5, color='red')
        ax.add_patch(true_state_circle)
        dy = torch.sin(states[0,0,2]).item() * 1.0
        dx = torch.cos(states[0,0,2]).item() * 1.0
        ax.arrow(starting_x, starting_y, dx, dy, color='red', width=0.15)

        # Plot the true next location
        true_x = states[0,1,0].item()
        true_y = states[0,1,1].item()
        true_state_circle = plt.Circle((true_x, true_y), 0.5, color='blue')
        ax.add_patch(true_state_circle)
        dy = torch.sin(states[0,1,2]).item() * 1.0
        dx = torch.cos(states[0,1,2]).item() * 1.0
        ax.arrow(true_x, true_y, dx, dy, color='blue', width=0.15)
