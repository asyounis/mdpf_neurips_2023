...
class SyntheticDiskTrackingProposalEvaluations(ProposalEvaluationBase):
    ...
    def render_experiment(self, ax, output_dict, observations, states):
        ...
        true_state_circle = plt.Circle((true_x, true_y), 0.05, color='green')
        ax.add_patch(true_state_circle)
        ...
        if(self.model.outputs_particles_and_weights() and self.render_particles):
            ...
            ax.scatter(x,y, color="black", s=1, zorder=100)
    ...
    #     # Plot the true starting location
    #     starting_x = states[0,0,2].item()
    #     starting_y = states[0,0,3].item()
    #     true_state_circle = plt.Circle((starting_x, starting_y), 0.05, color='red')
    #     ax.add_patch(true_state_circle)

    #     # Plot the true next location
    #     true_x = states[0,1,2].item()
    #     true_y = states[0,1,3].item()
    #     true_state_circle = plt.Circle((true_x, true_y), 0.05, color='green')
    #     ax.add_patch(true_state_circle)
    ...
    #         # Draw the particles
    #         x = particles[0, :, 2].cpu().numpy()
    #         y = particles[0, :, 3].cpu().numpy()
    #         ax.scatter(x,y, color="black", s=1, zorder=100)
