...
class ReferencePatch:
    ...
    def load_patch(self):
        ...
        image_filepath = "{}/patches/{:06d}.png".format(self.directory, self.patch_frame_number)
...
class Subsequence:
    ...
    def get_return_dict(self):
        ...
        # Get the grouund truths.  We need to set the Nans to a valid number so we just set them to 0.
        # These numbers will get ignored so there is no worry with setting them to 0 but for the implementation 
        # We need to set them to 0 to prevent an error
        states = self.groundtruths.float().clone()
        states[torch.isnan(states)] = 0.0
    ...
    def load_image(self, directory, frame_index):
        ...
        # Create the image file path.  Note the image names start at 1 not 0 so add 1 to the image name 
        # to get the correct image
        image_filepath = "{}/images/{:06d}.png".format(directory, frame_index)
...
class UAV123Dataset(BaseDataset):
    ...
    def create_sequences(self, dataset_directory, dataset_sequences):
        ...
        # Slice each sequence into smaller sub-sequences
        all_subsequences = []
        for sequence in tqdm(dataset_sequences):
            sequence_directory = "{}/{}/".format(dataset_directory, sequence)
    ...
    def split_sequence_into_subsequence(self, sequence_directory):
        ...
        # Construct the not in frame from the ground truth.  We mark not in frame by a Nan value in the ground truths
        not_in_frame = (torch.isnan(groundtruths[:,0]))

        # Find the first frame that doesnt have an occluded
        first_in_view_frame = not_in_frame.tolist().index(False)
