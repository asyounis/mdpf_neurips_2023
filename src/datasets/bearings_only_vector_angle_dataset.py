...
class BearingsOnlyVectorAngleDataset(BaseDataset):
    def __init__(self, dataset_params, dataset_type):
        ...
        self.enable_sparse_ground_truth = get_parameter_safely("enable_sparse_ground_truth", dataset_params, "dataset_params")
        if(self.enable_sparse_ground_truth):
            self.sparse_ground_truth_keep_probibility = get_parameter_safely("sparse_ground_truth_keep_probibility", dataset_params, "dataset_params")
    ...
    def __getitem__(self, idx):
        ...
        return_dict = {}
        return_dict["states"] = self.car_states[idx]
        return_dict["observations"] = self.sensor_obs[idx]


        if(self.enable_sparse_ground_truth):
            return_dict["ground_truth_mask"] = self.ground_truth_keep_mask[idx]
        else:
            return_dict["ground_truth_mask"] = torch.full(size=(self.car_states[idx].shape[0],), fill_value=True)
    ...
    def load_from_save(self, dataset_params, dataset_type):
        ...
        if("enable_sparse_ground_truth" in all_data):
            enable_sparse_ground_truth = all_data["enable_sparse_ground_truth"]
        else:
            enable_sparse_ground_truth = False
        ...
        if(dynamics_skip_amount != self.dynamics_skip_amount):
            return False
        if(enable_sparse_ground_truth != self.enable_sparse_ground_truth):
            return False

        if(enable_sparse_ground_truth):
            sparse_ground_truth_keep_probibility = all_data["sparse_ground_truth_keep_probibility"]
            if(sparse_ground_truth_keep_probibility != self.sparse_ground_truth_keep_probibility):
                return False


            self.ground_truth_keep_mask = all_data["ground_truth_keep_mask"]
    ...
    def save_dataset(self):
        ...
        save_dict["enable_sparse_ground_truth"] = self.enable_sparse_ground_truth

        if(self.enable_sparse_ground_truth):
            save_dict["sparse_ground_truth_keep_probibility"] = self.sparse_ground_truth_keep_probibility
            save_dict["ground_truth_keep_mask"] = self.ground_truth_keep_mask
    ...
    def generate_dataset(self):
        ...
        # Convert the last dim from from angle to vectors that represent the angle
        car_states_vector_angle = np.zeros((car_states.shape[0], 4))
        car_states_vector_angle[:, 0] = car_states[:, 0]
        car_states_vector_angle[:, 1] = car_states[:, 1]
        car_states_vector_angle[:, 2] = np.cos(car_states[:, 2])
        car_states_vector_angle[:, 3] = np.sin(car_states[:, 2])
        car_states = car_states_vector_angle
        ...
        if(self.enable_sparse_ground_truth):
            self.ground_truth_keep_mask = torch.rand(size=(self.dataset_size,self.subsequence_length,))
            self.ground_truth_keep_mask = self.ground_truth_keep_mask < self.sparse_ground_truth_keep_probibility

            # Always keep the last state!
            self.ground_truth_keep_mask[:, -1] = True

        else:
            self.ground_truth_keep_mask = torch.full(size=(self.dataset_size,self.subsequence_length,), fill_value=True)
